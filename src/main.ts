...
function render() {
  ...
  </div><div id="software-notices-root"></div>`;
  renderNoticeLayer();
...
function renderNoticeLayer() {
  const root = document.querySelector<HTMLDivElement>("#software-notices-root");
  if (!root) return;
  root.innerHTML = softwareNoticeLayer();
  root.querySelectorAll<HTMLButtonElement>("[data-dismiss-notice]").forEach((button) => button.onclick = () => {
    softwareNotices = softwareNotices.filter((item) => item.id !== Number(button.dataset.dismissNotice));
    renderNoticeLayer();
  });
}
...
async function setupEvents() { if (eventReady) return; eventReady = true; await listen<Message>("software-notification", (event) => { const notice = { ...event.payload, id: ++noticeId }; softwareNotices = [...softwareNotices, notice].slice(-3); renderNoticeLayer(); window.setTimeout(() => { softwareNotices = softwareNotices.filter((item) => item.id !== notice.id); renderNoticeLayer(); }, 9000); }); }
