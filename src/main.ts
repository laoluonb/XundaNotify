...
function settings() { return `<header><div><div class="eyebrow">PREFERENCES</div><h1>设置</h1><p>管理服务端口、通知方式和更新选项。</p></div></header><section class="panel settings"><label>监听端口<input id="port" value="${snapshot.port}" type="number" min="1" max="65535"></label><label>通知方式<select id="mode"><option>Windows 通知</option><option>软件通知</option><option>同时通知</option></select></label><label class="check"><input type="checkbox" id="sound" checked>播放通知提示音</label><label class="check"><input type="checkbox" id="quiet">免打扰模式（仅记录，不弹窗）</label><div class="setting-divider"></div><div class="version-row"><div><span>当前版本</span><strong>v${CURRENT_VERSION}</strong></div><button class="ghost" id="check-update">${updateState.status === "checking" ? "检查中..." : "检查更新"}</button></div><div class="update-result ${updateState.status}">${updateState.status === "available" ? `发现 v${esc(updateState.version || "")}，点击更新按钮打开下载页面。` : updateState.status === "latest" ? "已经是最新版本。" : updateState.note || ""}</div><button class="primary" id="save">保存设置</button></section>`; }
