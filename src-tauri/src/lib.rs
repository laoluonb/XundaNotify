...
fn parse_message(raw: &str) -> (String, String) {
    ...
    if package.is_empty() && sender.is_empty() && content.is_empty() {
        for pair in input.split('&') { let mut it=pair.splitn(2,'='); let k=it.next().unwrap_or("").to_lowercase(); let v=clean_value(it.next().unwrap_or("")); match k.as_str(){"package"|"pkg"|"from"|"app_package"=>package=v,"sender"|"nickname"|"name"|"title"|"通知标题"=>sender=v,"group"|"group_name"|"chat"|"conversation"|"群名"=>group=v,"content"|"msg"|"message"|"text"|"body"|"通知内容"=>content=v,_=>{}} }
    }
    if content.is_empty() && input.contains(':') && !input.contains("=") { content=input.to_string(); }
    let mut lines: Vec<String> = content.lines().map(str::trim).filter(|line| !line.is_empty()).filter(|line| !line.to_lowercase().starts_with("uid") && !line.to_lowercase().starts_with("timestamp") && !line.contains("com.tencent.")).map(str::to_string).collect();
    if sender.is_empty() && !lines.is_empty() { let first=lines[0].clone(); if first.contains('：') || first.contains(':') { let mut split=first.splitn(2, |c| c=='：' || c==':'); sender=split.next().unwrap_or("").trim().to_string(); lines[0]=split.next().unwrap_or("").trim().to_string(); } }
    if lines.last().map(|line| line == &sender).unwrap_or(false) { lines.pop(); }
    ...
    let title = if software.is_empty(){ sender.clone() } else if sender.is_empty(){ if group.is_empty(){software.to_string()}else{format!("{}-{}",software,group)} } else if group.is_empty(){ format!("{}-{}", software, sender) } else { format!("{}-{}-{}", software, group, sender) };
